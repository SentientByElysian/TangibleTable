...
public class Surface : MonoBehaviour {
	...
	// Events for object state changes
	public event Action<SurfaceObject, Vector2, float> OnObjectAdded;
	public event Action<SurfaceObject, Vector2, float> OnObjectUpdated;
	public event Action<SurfaceObject> OnObjectRemoved;
	...
	public List<int> puckIDs = new List<int>();

	// Dictionary to track active objects and their previous positions
	private Dictionary<int, bool> activeObjects = new Dictionary<int, bool>();
	...
	private void Init(int spawnObjectCount) {
		...
		activeObjects = new Dictionary<int, bool>();
	...
	public void ResetSurfaceObjects() {
		foreach (var objectId in activeObjects.Keys) {
			SurfaceObject so = surfaceObjects.Find(s => s.id == objectId);
			if (so != null) {
				OnObjectRemoved?.Invoke(so);
			}
		}
	...
	private void UpdateSurfaceObject(ObjectPointer pointer, bool moveToTop = false) {
		...
		if (activeObjects.TryAdd(so.id, true)) {
			OnObjectAdded?.Invoke(so, pointer.Position, angle);
		} else {
			OnObjectUpdated?.Invoke(so, pointer.Position, angle);
		}
	}
		...
		foreach (Pointer pointer in e.Pointers) {
			if (pointer.InputSource is TuioInput) {
				if (pointer.Type == Pointer.PointerType.Touch) {
					...
					ObjectPointer objPointer = pointer as ObjectPointer;
					UpdateSurfaceObject(objPointer);
					HandleObjectRemoved(objPointer.ObjectId);
				}
