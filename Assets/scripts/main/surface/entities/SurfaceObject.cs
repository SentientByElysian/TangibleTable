...
public class SurfaceObject : MonoBehaviour, ISurfaceEntity {
	...
	public void Init(int id, int index) {
		...
		if (index < sprites.Count) spriteRenderer.sprite = sprites[index];
		if (index < colors.Count) spriteRenderer.color = colors[index];
	}

	[ContextMenu("Change Object ID")]
	public void ChangeObjectID()
	{
		tuioObject = new TUIOObject(tuioObject.Id,_id);
	}
