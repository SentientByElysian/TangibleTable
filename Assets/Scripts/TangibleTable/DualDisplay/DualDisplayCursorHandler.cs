...
{
    /// <summary>
    /// Handles TUIO cursor interactions and converts them to Unity events.
    /// Can be used to handle clicks and interact with UI elements.
    /// 
    /// Setup requirements:
    /// - Attach this to a GameObject with a CustomTuio11Visualizer component
    /// - Make sure your main camera has a PhysicsRaycaster component
    /// - Ensure your scene has an EventSystem
    /// </summary>
    public class DualDisplayCursorHandler : MonoBehaviour
    {
        [Header("References")]
        [SerializeField] private DualDisplayTuio11Visualizer _tuioVisualizer;
        ...
        private void Awake()
        {
            ...
            {
                _tuioVisualizer = GetComponent<DualDisplayTuio11Visualizer>();
                if (_tuioVisualizer == null)
                {
                    Debug.LogError("No CustomTuio11Visualizer found on this GameObject. The cursor handler won't work.");
                    enabled = false;
                    return;
                }
            }
